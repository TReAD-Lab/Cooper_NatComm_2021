* Encoding: UTF-8.
*Include all of the syntax for analyses in r00 MRS manuscript submitted to nature communications. 
*separate files include syntax used for analysis on ICCs and bootstrapped correlations to generate values for figures. 
 
*average age and PSS for demographics table.
*note that group 1 is mclean stress, 2 is no stress control, 3 is emory stress, 4 is patients. 
*note that other demographics (race, eduation, etc) are not incldued in the SPSS doc and need to be checked separately.  
USE ALL. 
