...
*Supplemental analysis - correlations between PSS, cortisol percent change, or VAMS percent change (percent change T1 to T3 in figure, T2 and T4 in names here). 
USE ALL. 
COMPUTE filter_$=(Good_MRS = 1 & group ~= 2 & group ~=4). 
VARIABLE LABELS filter_$ 'Good_MRS = 1 & group ~= 2 & group ~=4 (FILTER)'. 
VALUE LABELS filter_$ 0 'Not Selected' 1 'Selected'. 
...
*comparison of each metabolite (and get means and sds) for supplemental tables 2a-2d. 
USE ALL. 
...
*values for supplemental table 3 (loss PSS metabolite effects). 
SPLIT FILE OFF. 
USE ALL. 
...
*NOTE- values for supplementary tables 4-12 are all from regression models above and in the syntax file for the main manuscript. 
...
*NAA without influential datapoints for supplemental table 2 note. 
